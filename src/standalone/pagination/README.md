...
- Disabled controls get:
  ...
  - `aria-disabled="true"`
- In numbered mode, `prev` is visually hidden on the first page and `next` is visually hidden on the last page (using `visibility: hidden`, so layout space stays reserved).
